...
module namespace muURL="http://markup.nz/#muURL";
...
import module namespace muCache = "http://markup.nz/#muCache" at 'muCache.xqm';
...
let $input :=
	if( not( contains( $u, ':')) )
	    then (
		fn:error(fn:QName('http://markup.nz/#muURL',
				  'urlHasNoScheme'),
...
(:~
@param $url
@return map of URL parms key values
:)
declare
function muURL:urlParams( $URL as xs:string ) as map(*){
let $map := map {}
  for $param in tokenize( muURL:urlQuery( $URL ), '&amp;' )
